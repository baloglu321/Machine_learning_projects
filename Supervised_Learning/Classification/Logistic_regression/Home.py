...
def start_streamlit():
    ...
    st.header("Customer Segment Classification with Logistic regression", divider="blue")
    col1, col2 = st.columns(2)
    with col1:
        gender = st.selectbox("Select Gender", tuple(string_unq["Gender"]))
        married = st.selectbox(
            "Maried ?", tuple(string_unq["Ever_Married"])
        )
    ...
    with col1:
        age = st.slider(
            "Age", int(min_values["Age"]), int(max_values["Age"])
        )
       
    with col2:
        experience = st.slider(
            "Work Experience", int(min_values["Work_Experience"]), int(max_values["Work_Experience"])
        )
    with col3:
        family_size = st.slider("Family Size", int(min_values["Family_Size"]), int(max_values["Family_Size"]))
    st.divider()
