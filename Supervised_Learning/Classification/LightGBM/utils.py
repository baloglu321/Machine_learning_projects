...
def predict(In_values):
    ...
    prediction = np.argmax(model.predict(pred_data), axis=1)
...
def update_model():
    ...
    lgb_train = lgb.Dataset(X_train_OHE, label=y_train_encode)
    params = {
    ...
    y_pred = np.argmax(lgb_model.predict(X_test_OHE), axis=1)
