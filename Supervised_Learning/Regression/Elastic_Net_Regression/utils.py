...
def predict_next_day_price(low, high, open_price, close_price, model):
    ...
    data = pd.DataFrame(
        data={"low": low, "high": high, "open": open_price, "close": close_price}
    )
...
def calc_hist():
    ...
    for index in range(100, len(data)):
        ...
        predict = predict_next_day_price(
            data["Low"][index],
            data["High"][index],
            data["Open"][index],
            data["Close"][index],
...
def update_model():
    ...
    elestic_net_coefficients = pd.DataFrame(
        pd.Series(model.coef_, index=["Low", "High", "Open", "Close"]),
        columns=["Value"],
    )
    return model, mse, r2, elestic_net_coefficients
