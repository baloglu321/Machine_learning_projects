...
def convert_pred_data(data):
    if check_is_dir("models/"):
        ...
        encoded_labels = encoder.transform(data)
...
def update_model():
    ...
    X = data.drop('Price', axis=1)  # Mean number of room, percentage of population with low status,average number of people
    y = data['Price']
    ...
    with open('model/model_performance.txt', 'w') as file:
